    ...
    if (req.isAuthenticated()){
        ...
        if (newDrink.tags){
            ...
            (async () => {
                ...
                const { rows } = await client.query("INSERT INTO recipes (recipe_name, garnish, notes, glass_id, ice_id, user_id, tags) VALUES ($1, $2, $3, $4, $5, $6, ARRAY ["+array_to_send+"]) RETURNING recipe_id", [newDrink.name, newDrink.garnish, newDrink.notes, newDrink.glass, newDrink.ice, req.user.id])
            ...
            (async () => {
                ...
                try {
                await client.query('BEGIN')
                const { rows } = await client.query("INSERT INTO recipes (recipe_name, garnish, notes, glass_id, ice_id, user_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING recipe_id", [newDrink.name, newDrink.garnish, newDrink.notes, newDrink.glass, newDrink.ice, req.user.id])
                for (ingredient of newDrink.ingredients){
                    let insertIngredientsText = `INSERT INTO "ingredients" ("ingredient_name", "ingredient_quantity", "recipe_id") VALUES ($1, $2, $3)`;
                    let insertIngredientsValues = [ingredient.name, ingredient.quantity, rows[0].recipe_id];
                    await client.query(insertIngredientsText, insertIngredientsValues)
    ...
    if(req.isAuthenticated()){  
        ...
        if (editedDrink.tags){
            ...
            (async () => {
                ...
                await client.query("UPDATE recipes SET recipe_name = $1, garnish = $2, notes = $3, glass_id = $4, ice_id = $5, tags = ARRAY ["+array_to_send+"] WHERE recipe_id=$6 AND user_id=$7", [editedDrink.recipe_name, editedDrink.garnish, editedDrink.notes, editedDrink.glass_id, editedDrink.ice_id, editedDrink.recipe_id, req.user.id])
                await client.query('COMMIT')
                } 
                catch (e) {
                await client.query('ROLLBACK')
            ...
            (async () => {
                ...
                await client.query("UPDATE recipes SET recipe_name = $1, garnish = $2, notes = $3, glass_id = $4, ice_id = $5 WHERE recipe_id=$6 AND user_id=$7", [editedDrink.recipe_name, editedDrink.garnish, editedDrink.notes, editedDrink.glass_id, editedDrink.ice_id, editedDrink.recipe_id, req.user.id])
                await client.query('COMMIT')
                } 
                catch (e) {
                await client.query('ROLLBACK')
